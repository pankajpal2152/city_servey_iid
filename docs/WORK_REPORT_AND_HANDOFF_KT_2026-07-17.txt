# Work Report and Backend Handoff KT

Date: 2026-07-17

## Work Completed Today

Implemented and verified the current City Survey backend API scope in the
existing Node.js application.

Completed items:

- Integrated `DEV_CITY_SURVEY` RDS database credentials through `.env`.
- Kept database config source-driven by environment variables in `config/db.js`.
- Implemented active authenticate API:
  - `POST /api/auth/api-post-authenticate-user`
  - Procedure: `USP_POST_USER_AUTHENTICATE_ACTIVITY`
- Implemented active add/update user API:
  - `POST /api/user/api-post-add-update-user`
  - Procedure: `USP_POST_USER_PROFILE_ACTIVITY`
- Implemented active user list info API:
  - `GET /api/user/api-get-view-user-list-info`
  - Procedure: `USP_GET_USER_LIST_ACTIVITY`
- Implemented active add/update customer details API:
  - `POST /api/customer/api-post-add-update-customer-details`
  - Procedure: `USP_POST_CUSTOMER_DETAILS_ACTIVITY`
- Implemented active customer list details API:
  - `GET /api/customer/api-get-view-list-customer-details`
  - Procedure: `USP_GET_LIST_CUSTOMER_ACTIVITY`
- Implemented active specific customer details API:
  - `GET /api/customer/api-get-view-specific-customer-details`
  - Procedure: `USP_GET_SPECIFIC_CUSTOMER_ACTIVITY`
- Implemented active add/update project API:
  - `POST /api/project/api-post-add-update-project`
  - Procedure: `USP_POST_PROJECT_ACTIVITY`
- Implemented active project list details API:
  - `GET /api/project/api-get-view-list-project-details`
  - Procedure: `USP_GET_LIST_PROJECT_ACTIVITY`
- Implemented active specific project details API:
  - `GET /api/project/api-get-view-specific-project-details`
  - Procedure: `USP_GET_SPECIFIC_PROJECT_ACTIVITY`
- Implemented active customer-wise project details API:
  - `GET /api/project/api-get-view-specific-customer-wise-project-details`
  - Procedure: `USP_GET_SPECIFIC_CUSTOMER_WISE_PROJECT_ACTIVITY`
- Implemented active master system role list API:
  - `GET /api/master/api-get-view-master-system-roles`
  - Procedure: `USP_GET_ALL_MASTER_DATA`
- Implemented active add/update master system role API:
  - `POST /api/master/api-post-add-update-master-system-role`
  - Procedure: `USP_POST_ALL_MASTER_DATA`
- Implemented active master survey type list API:
  - `GET /api/master/api-get-view-master-survey-type`
  - Procedure: `USP_GET_ALL_MASTER_DATA`
- Implemented active add/update master survey type API:
  - `POST /api/master/api-post-add-update-master-survey-type`
  - Procedure: `USP_POST_ALL_MASTER_DATA`
- Implemented active master property type list API:
  - `GET /api/master/api-get-view-master-property-type`
  - Procedure: `USP_GET_ALL_MASTER_DATA`
- Implemented active add/update master property type API:
  - `POST /api/master/api-post-add-update-master-property-type`
  - Procedure: `USP_POST_ALL_MASTER_DATA`
- Implemented active master details API:
  - `GET /api/master/api-get-view-master-details`
  - Procedure: `USP_GET_ALL_MASTER_DATA`
- Merged senior developer health API:
  - `GET /health`
  - Route: `routes/health.js`
- Added JWT generation for successful authentication.
- Added JWT protection for user list, add/update user, and customer details.
- Added JWT protection for add/update project.
- Added JWT protection for project list details.
- Added JWT protection and `PROJECT_SYS_ID` validation for specific project details.
- Added JWT protection and `CUSTOMER_SYS_ID` validation for customer-wise project details.
- Added JWT protection for master system role list and add/update.
- Added response helper for stored procedure `JSON_VALUE` parsing.
- Disabled active mounting of old event, attendee, payment, file upload, and
  email APIs from `index.js`.
- Limited Swagger generation to the active City Survey route files.
- Updated Swagger server URL to relative `/` so Swagger calls the current
  running port instead of any hardcoded localhost port.
- Added startup handling for `EADDRINUSE` port conflicts.
- Updated README and added this documentation pack.

## Important Technical Finding

The database developer's sample for `USP_POST_USER_PROFILE_ACTIVITY` showed a
plain JSON object, but the live stored procedure reads fields using `$[0]`.

Backend decision:

- Frontend can send a normal object.
- Backend wraps the object as a one-item array before calling the stored
  procedure.

Implemented in:

- `routes/user.js`

## Files Changed or Added

Backend files:

- `config/db.js`
- `index.js`
- `middlewares/auth.js`
- `routes/auth.js`
- `routes/user.js`
- `routes/customer.js`
- `routes/project.js`
- `routes/master.js`
- `routes/health.js`
- `utils/procedureResponse.js`
- `docs/swagger.js`
- `config/test.js`

Documentation files:

- `README.md`
- `docs/INDEX.md`
- `docs/MODULE_ARCHITECTURE_FLOW.md`
- `docs/BROWSER_DEMO_AND_DEMO_FLOW.md`
- `docs/DEV_SERVER_DEPLOYMENT_CHECKLIST.md`
- `docs/RUN_AND_TEST_GUIDE.md`
- `docs/API_URLS.md`
- `docs/API_ENDPOINTS_DEV_SERVER.md`
- `docs/WORK_REPORT_AND_HANDOFF_KT_2026-07-17.md`
- `docs/UI_DEVELOPER_HANDOFF_KT.md`

## Verification Done

Syntax checks:

- `node --check index.js`
- `node --check routes/auth.js`
- `node --check routes/user.js`
- `node --check routes/customer.js`
- `node --check routes/project.js`
- `node --check routes/master.js`
- `node --check routes/health.js`
- `node --check middlewares/auth.js`
- `node --check utils/procedureResponse.js`
- `node --check docs/swagger.js`

Database checks:

- Connected to `DEV_CITY_SURVEY`
- Confirmed procedure exists:
  - `USP_POST_USER_AUTHENTICATE_ACTIVITY`
  - `USP_GET_USER_LIST_ACTIVITY`
  - `USP_POST_USER_PROFILE_ACTIVITY`
  - `USP_GET_LIST_CUSTOMER_ACTIVITY`
  - `USP_GET_SPECIFIC_CUSTOMER_ACTIVITY`
  - `USP_POST_CUSTOMER_DETAILS_ACTIVITY`
  - `USP_GET_LIST_PROJECT_ACTIVITY`
  - `USP_GET_SPECIFIC_PROJECT_ACTIVITY`
  - `USP_GET_SPECIFIC_CUSTOMER_WISE_PROJECT_ACTIVITY`
  - `USP_POST_PROJECT_ACTIVITY`
  - `USP_GET_ALL_MASTER_DATA`
  - `USP_POST_ALL_MASTER_DATA`
- Auth procedure returned `status: true` for sample user.

HTTP API smoke test:

- Started local server on configured local port `3103`
- Health API returns HTTP 200 without JWT
- Auth API returned HTTP 200 and token
- User list API returns read-only user data when records exist
- Protected add/update API accepted token
- Customer list API returns read-only customer data with nested `PIC_DETAILS`
- Specific customer API returns read-only customer profile/details data for a
  valid customer id
- Existing sample email returned `This Email Id Already Exists`
- Sample email count stayed `1 -> 1`, so no duplicate test user was created
- Customer route validation can be tested safely with `{ "ITEM": "ADD" }`;
  full customer samples are write operations.
- Project route validation can be tested safely with `{ "ITEM": "ADD" }`;
  full project samples are write operations.
- Project list route can be tested safely with `ITEM=VIEW_ALL`; it is a
  read-only API.
- Specific project route can be tested safely with `ITEM=SPECIFIC` and
  `PROJECT_SYS_ID=2`; it is a read-only API.
- Customer-wise project route can be tested safely with `ITEM=SPECIFIC` and
  `CUSTOMER_SYS_ID=4`; it is a read-only API.
- Current customer-wise project DB verification reaches
  `USP_GET_SPECIFIC_CUSTOMER_WISE_PROJECT_ACTIVITY`, but the procedure returns
  `ERRNO=1054` with `Unknown column 'PI.PROJECT_ADDRESS' in 'field list'`.
- System role list route can be tested safely with `ITEM=VIEW_ALL`; it is a
  read-only API.
- System role add/update route validation can be tested safely with
  `{ "ITEM": "ADD" }`; full system role samples are write operations.
- Survey type list route can be tested safely with `ITEM=VIEW_ALL`; it is a
  read-only API.
- Survey type add/update route validation can be tested safely with
  `{ "ITEM": "ADD" }`; full survey type samples are write operations.
- Property type list route can be tested safely with `ITEM=VIEW_ALL`; it is a
  read-only API.
- Property type add/update route validation can be tested safely with
  `{ "ITEM": "ADD" }`; full property type samples are write operations.
- Master details route can be tested safely with `ITEM=VIEW_ALL`; it is a
  read-only API.

## Current Working API Contracts

### Authenticate User

Endpoint:

`POST /api/auth/api-post-authenticate-user`

Request:

```json
[
  {
    "USER_NAME": "sai@yopmail.com",
    "PASSWORD": "Abc@1234"
  }
]
```

Response:

- Returns DB user data
- Returns `Token` when `status` is `true`

### Add or Update User

Endpoint:

`POST /api/user/api-post-add-update-user`

Header:

```text
Authorization: Bearer <Token>
```

Request:

```json
{
  "ITEM": "ADD_USER",
  "USER_SYS_ID": 0,
  "SYSTEM_ROLE_SYS_ID": 1,
  "FIRST_NAME": "Sai",
  "LAST_NAME": "Roy",
  "GENDER": "Male",
  "MOBILE_NO": "7894561230",
  "EMAIL_ID": "sai@yopmail.com",
  "ORGANISATION_SYS_ID": "1",
  "PASSWORD": "Abc@1234",
  "CREATED_BY": "1"
}
```

### View User List Info

Endpoint:

`GET /api/user/api-get-view-user-list-info`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_USER_LIST_ACTIVITY('VIEW_USER', 'VIEW_ALL', @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
```

### Add or Update Customer Details

Endpoint:

`POST /api/customer/api-post-add-update-customer-details`

Header:

```text
Authorization: Bearer <Token>
```

Request:

```json
{
  "ITEM": "ADD",
  "CUSTOMER_TYPE": "Individual",
  "CUSTOMER_NAME": "Ravi",
  "BILLING_ADDRESS": "Kolkata",
  "CREATED_BY": 1,
  "PIC": [
    {
      "FULL_NAME": "Ranjan",
      "CONTACT_NO": "7875585",
      "EMAIL_ID": "ranjan@gmail.com",
      "DEPARTMENT": "Back Office",
      "DESIGNATION": "Employee"
    }
  ]
}
```

### View Customer List Details

Endpoint:

`GET /api/customer/api-get-view-list-customer-details`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_LIST_CUSTOMER_ACTIVITY('VIEW_CUSTOMER', 'VIEW_ALL', @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
```

### View Specific Customer Details

Endpoint:

`GET /api/customer/api-get-view-specific-customer-details`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_SPECIFIC_CUSTOMER_ACTIVITY('VIEW_CUSTOMER', 'PROFILE', 4, @ERRNO, @ERRMSG);
```

Required query:

```text
CUSTOMER_SYS_ID=4
```

Default query:

```text
ITEM=PROFILE
```

### View Master System Roles

Endpoint:

`GET /api/master/api-get-view-master-system-roles`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_ALL_MASTER_DATA('VIEW_SYSTEM_ROLE', 'VIEW_ALL', '0', 0, @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
RECORD_SYS_ID=0
ORGANIZATION_SYS_ID=0
```

### Add or Update Master System Role

Endpoint:

`POST /api/master/api-post-add-update-master-system-role`

Header:

```text
Authorization: Bearer <Token>
```

Request:

```json
{
  "ITEM": "ADD",
  "RECORD_SYS_ID": "0",
  "SYSTEM_ROLE": "Business Admin",
  "ROLE_DESC": "Business Admin",
  "USER_TYPE": "Internal",
  "CREATED_BY": 1
}
```

### View Master Survey Type

Endpoint:

`GET /api/master/api-get-view-master-survey-type`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_ALL_MASTER_DATA('VIEW_SURVEY_TYPE', 'VIEW_ALL', 0, 0, @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
RECORD_SYS_ID=0
ORGANIZATION_SYS_ID=0
```

### Add or Update Master Survey Type

Endpoint:

`POST /api/master/api-post-add-update-master-survey-type`

Header:

```text
Authorization: Bearer <Token>
```

Request:

```json
{
  "ITEM": "ADD",
  "RECORD_SYS_ID": "0",
  "SURVEY_TYPE": "Seminer",
  "CREATED_BY": "1"
}
```

### View Master Property Type

Endpoint:

`GET /api/master/api-get-view-master-property-type`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_ALL_MASTER_DATA('VIEW_PROPERTY_TYPE', 'VIEW_ALL', 0, 0, @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
RECORD_SYS_ID=0
ORGANIZATION_SYS_ID=0
```

### Add or Update Master Property Type

Endpoint:

`POST /api/master/api-post-add-update-master-property-type`

Header:

```text
Authorization: Bearer <Token>
```

Request:

```json
{
  "ITEM": "ADD",
  "RECORD_SYS_ID": "0",
  "PROPERTY_TYPE": "Seminer",
  "CREATED_BY": "1"
}
```

### View Master Details

Endpoint:

`GET /api/master/api-get-view-master-details`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_ALL_MASTER_DATA('VIEW_MASTER_NAME', 'VIEW_ALL', '0', 0, @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
RECORD_SYS_ID=0
ORGANIZATION_SYS_ID=0
```

### Add or Update Project

Endpoint:

`POST /api/project/api-post-add-update-project`

Header:

```text
Authorization: Bearer <Token>
```

Request:

```json
{
  "ITEM": "ADD",
  "CUSTOMER_SYS_ID": 17,
  "PIC_SYS_ID": 22,
  "SITE_ADDRESS": "51 Jalan Kukoh 01-37, 1620051, Singapore",
  "ASSIGNED_SURVEYOR_SYS_ID": 1,
  "ASSIGNED_SURVEYOR": "Marcus Koh",
  "SURVEY_TYPE_SYS_ID": 5,
  "PROPERTY_TYPE_SYS_ID": 15,
  "CREATED_BY": 1
}
```

### View Project List Details

Endpoint:

`GET /api/project/api-get-view-list-project-details`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_LIST_PROJECT_ACTIVITY('VIEW_PROJECT', 'VIEW_ALL', @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=VIEW_ALL
```

### View Specific Project Details

Endpoint:

`GET /api/project/api-get-view-specific-project-details`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_SPECIFIC_PROJECT_ACTIVITY('VIEW_PROJECT', 'SPECIFIC', 2, @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=SPECIFIC
PROJECT_SYS_ID=2
```

### View Customer-Wise Project Details

Endpoint:

`GET /api/project/api-get-view-specific-customer-wise-project-details`

Header:

```text
Authorization: Bearer <Token>
```

Stored procedure call:

```sql
CALL USP_GET_SPECIFIC_CUSTOMER_WISE_PROJECT_ACTIVITY('VIEW_CUSTOMER_WISE_PROJECT', 'SPECIFIC', 4, @ERRNO, @ERRMSG);
```

Default query:

```text
ITEM=SPECIFIC
CUSTOMER_SYS_ID=4
```

### Health Check

Endpoint:

`GET /health`

Response:

```json
{
  "status": "true",
  "response": "City Survey API is running",
  "uptime": 1.23,
  "timestamp": "2026-07-17T00:00:00.000Z"
}
```

## Handoff KT for Backend Developer

Walkthrough order:

1. Start at `index.js`.
2. Show only active City Survey routes are mounted.
3. Show `config/db.js` reads all DB config from `.env`.
4. Show `routes/auth.js`:
   - normalizes auth payload
   - calls auth procedure
   - parses `JSON_VALUE`
   - signs JWT
5. Show `middlewares/auth.js`:
   - expects `Authorization: Bearer <token>`
   - returns 401 for missing token
   - returns 403 for invalid token
6. Show `routes/user.js`:
   - requires JWT middleware
   - calls user list procedure for view/list
   - validates `ITEM`
   - wraps payload into array for DB
   - calls user profile procedure
7. Show `routes/customer.js`:
   - requires JWT middleware
   - calls customer list procedure for view/list
   - validates `CUSTOMER_SYS_ID` and calls specific customer procedure
   - validates `ITEM` and required `ADD` fields
   - calls customer details procedure
8. Show `routes/project.js`:
   - requires JWT middleware
   - calls project list procedure for view/list
   - validates `PROJECT_SYS_ID` and calls specific project procedure
   - validates `CUSTOMER_SYS_ID` and calls customer-wise project procedure
   - validates `ITEM` and required `ADD` fields
   - calls project procedure
9. Show `routes/master.js`:
   - requires JWT middleware
   - calls master list procedure for system roles
   - validates `ITEM` and required `ADD` fields
   - calls master add/update procedure for system roles
10. Show `routes/health.js`:
   - returns API process status without DB or JWT
11. Show `utils/procedureResponse.js`:
   - parses procedure response safely
12. Show Swagger at `/api/api-docs`.
13. Run health, auth, user, customer list, project list, specific project
    details, customer-wise project details, system role list, safe customer validation, safe project
    validation, and safe system-role validation demo.

## Handoff KT for Database Developer

Confirmed:

- Backend connects to `DEV_CITY_SURVEY`.
- Backend calls stored procedures using parameterized `conn.execute`.
- Backend uses operation names exactly as provided:
  - `AUTHENTICATE_USER`
  - `ADD_UPDATE_USER`
  - `VIEW_CUSTOMER`
  - `ADD_UPDATE_CUSTOMER`
  - `VIEW_PROJECT`
  - `VIEW_CUSTOMER_WISE_PROJECT`
  - `ADD_UPDATE_PROJECT`
  - `VIEW_SYSTEM_ROLE`
  - `ADD_UPDATE_SYSTEM_ROLE`
  - `VIEW_SURVEY_TYPE`
  - `ADD_UPDATE_SURVEY_TYPE`
  - `VIEW_PROPERTY_TYPE`
  - `ADD_UPDATE_PROPERTY_TYPE`
  - `VIEW_MASTER_NAME`

DB follow-up points:

- Fix `USP_GET_SPECIFIC_CUSTOMER_WISE_PROJECT_ACTIVITY`; current dev response
  is `ERRNO=1054` and `Unknown column 'PI.PROJECT_ADDRESS' in 'field list'`.
- Confirm final JSON shape for `USP_POST_USER_PROFILE_ACTIVITY`. Current live
  procedure uses `$[0]`, so backend sends array shape to DB.
- Confirm update-user payload fields for `ITEM: UPDATE_USER`.
- Confirm whether password should be mandatory for update.
- Confirm whether future read/list user APIs will be provided.
- Confirm final update/delete customer payload fields and expected responses.
- Confirm final update/delete project payload fields and expected responses.
- Confirm final update/delete system role payload fields and expected responses.
- Confirm standard response shape for success and failure across procedures.

## Known Notes and Risks

- Port `3000` was already occupied locally during verification, so temporary
  testing was moved to local port `3103`.
- The stored procedure commits internally, so write tests create real dev DB
  records.
- There are no automated unit/integration test files yet.
- Legacy route files still exist in the repo, but they are not mounted.

## Recommended Next Backend Tasks

- Add automated monitoring that calls `GET /health` during deployments.
- Add automated integration test script under a `tests` or `scripts` folder.
- Confirm update-user and delete-user DB contracts.
- Confirm update/delete customer DB contracts.
- Confirm update/delete project DB contracts.
- Confirm update/delete system role DB contracts.
- Remove or archive legacy route files after senior developer approval.
- Keep `.env.example` updated and ensure `.env` stays ignored before pushing to
  remote.
