...
// ─────────────────────────────────────────────────────────────────────────────
// AUTO-DETECT system Chromium — checked once at startup
// Priority: PUPPETEER_EXECUTABLE_PATH → CHROMIUM_PATH → common paths
// ─────────────────────────────────────────────────────────────────────────────
function findChromiumExecutable() {
  ...
  for (const p of candidates) {
    if (p && fs.existsSync(p)) {
      console.log(`[BadgePdfMailer] Chromium found: ${p}`);
      return p;
    }
  }

  console.warn('[BadgePdfMailer] ⚠️  No system Chromium found in common paths — Puppeteer will try bundled binary');
  return null;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// PDF RENDERER — uses system Chromium if found, otherwise Puppeteer bundled
// ─────────────────────────────────────────────────────────────────────────────
  ...
  try {
    const launchOptions = {
      ...
      args: [
        ...
        '--disable-gpu',          // important on ARM/headless servers
        '--disable-web-security',
        '--disable-features=IsolateOrigins,site-per-process',
      ],
    };

    // Use system Chromium if detected at startup — skips bundled x64-only binary
    if (CHROMIUM_EXECUTABLE) {
      launchOptions.executablePath = CHROMIUM_EXECUTABLE;
    }

    console.log(`[BadgePdfMailer] Launching: ${CHROMIUM_EXECUTABLE || 'puppeteer bundled'}`);
